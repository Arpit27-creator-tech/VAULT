      ...
      >
        ...
        <AnimatePresence mode="wait">
          {currentStep.centered || !spotlightRect ? (
            <div 
              key={`centered-${step}`}
              className="fixed inset-0 z-10 flex items-center justify-center p-3 sm:p-6 pointer-events-none"
            >
              <motion.div
                ...
                <div className="bg-[#071E14]/97 backdrop-blur-2xl border-[3px] border-[#03140C] shadow-[8px_8px_0px_#020C07,0_0_40px_rgba(16,185,129,0.15)] p-5 sm:p-6 max-h-[88vh] overflow-y-auto">
                  ...
                  {currentStep.id === 'id-card' && (
                    <div className="my-3 rounded-2xl border-2 border-[#03140C] bg-[#020B06] overflow-hidden shadow-inner flex flex-col">
                      <div className="bg-[#051C12] px-3.5 py-2 border-b border-[#134830] flex items-center justify-between text-[11px] font-mono text-[#34D399]">
            ...
            <motion.div
              key={`spotlight-${step}`}
              initial={{ opacity: 0, y: tooltipAbove ? -16 : 16, scale: 0.97 }}
              animate={{ opacity: 1, y: 0, scale: 1 }}
              exit={{ opacity: 0, y: tooltipAbove ? 12 : -12, scale: 0.97 }}
              transition={{ duration: 0.25, ease: 'easeOut' }}
              className="absolute z-10 w-[92vw] max-w-md"
              style={(() => {
